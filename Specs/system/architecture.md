...
flowchart LR
    ...
    subgraph workflows[GitHub Actions workflows]
        ...
        opening[load_opening_hours.yml<br/>daily - planned]
    ...
    subgraph repo[Repo files]
        ...
        wxRaw[(weather_raw/)]
        opRaw[(pool_opening_raw/<br/>planned)]
        hol[(holidays/)]
    ...
    style opening stroke-dasharray: 5 5
    style opRaw stroke-dasharray: 5 5
```

Dashed boxes are **planned** (see
...
| `pool_opening_raw/` *(planned)* | External opening-hours scraper | daily | `pool_opening_YYYYMMDD_HHMMSS.json` |
...
3. Apply facility-alias resolution (see
   [facility-name-aliases](../changes/facility-name-aliases/proposal.md)).
...
Forecast and historical CSVs share an identical schema — see
[forecast-file-format](../changes/forecast-file-format/proposal.md).

**Planned change** ([integrate-opening-hours](../changes/integrate-opening-hours/proposal.md)):
after model prediction, apply a deterministic overlay from the
opening-hours snapshot that sets `is_open = 0` and `occupancy_percent = 0`
on closed hours.

Trigger: `forecast.yml` runs daily (05:00 UTC).
...
| Raw opening hours *(planned)* | `pool_opening_YYYYMMDD_HHMMSS.json` |
